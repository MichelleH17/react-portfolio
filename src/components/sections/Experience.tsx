...
export const Experience = () => {
  return (
    <section id="experience" className="pt-32 relative overflow-hidden text-primary">
      <div className="absolute top-1/2 left-1/4 w-96 h-96 bg-secondary/10 rounded-full blur-3xl -translate-1/2"/>
      <Container className="relative z-10">
        <div className="max-w-3xl mb-16">
          ...
          <h2 className="text-4xl md:text-5xl mt-4 mb-6 font-bold text-secondary leading-tight animate-fade-in animation-delay-100">
            Always learning,
        ...
        <div className="relative">
          <div className="absolute timeline-glow left-0 md:left-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-secondary via-secondary/50 to-transparent md:-translate-x-1/2" />
          <div className="space-y-12">
            {experiences.map((experience, idx) => (
              <div key={idx} className="relative grid md:grid-cols-2 gap-8 animate-fade-in" style={{ animationDelay: `${(idx + 1) * 150}ms` }}>

                {/* Dot */}
                <div className="absolute left-0 md:left-1/2 top-0 bg-secondary w-3 h-3 rounded-full -translate-x-1/2 ring-4 ring-surface z-10">
                  {experience.current && (
                ...
                <div className={`pl-8 md:pl-0 ${idx % 2 === 0 ? "md:pr-16 md:text-right" : "md:col-start-2 md:pl-16"}`}>
                  <div className="glass p-6 rounded-2xl border border-secondary/20 hover:border-secondary/70 transition-all duration-500">
                    <span className="text-sm text-secondary font-medium">{experience.period}</span>
                    <h3 className="text-xl font-semibold mt-2">{experience.role}</h3>
                    <p className="text-primary/70">{experience.company}</p>
                    <p className="text-sm text-primary/70 mt-4">{experience.description}</p>
                    <div className={`flex flex-wrap gap-2 mt-4 ${idx % 2 === 0 ? "md:justify-end" : ""}`}>
                      {experience.technologies.map((tech, techIdx) => (
                        <span key={techIdx} className="px-3 py-1 bg-secondary/5 text-secondary rounded-full text-xs">{tech}</span>
                      ))}                    
                    </div>
                  </div>
                </div>
